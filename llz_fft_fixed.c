...
typedef struct  _llz_fft_fixed_ctx_t {
    int base;
    int length;
    int *bit_rvs;
    int *fft_work;
    ...
    for (dl = size; dl > 1; dl >>= 1, astep += astep) {
        ...
        for (; l2 < end; l1 = l2, l2 = l2 + dl) {
            ...
            for (angle = 0; l1 < ol2; l1 += 2, l2 += 2) {
                ...
                yr = LLZ_FIXMUL_32X15(dr, wr) - LLZ_FIXMUL_32X15(di, wi);
                yi = LLZ_FIXMUL_32X15(dr, wi) + LLZ_FIXMUL_32X15(di, wr);
    ...
    for (dl = 2; astep > 0; dl += dl, astep >>= 1) {
        ...
        for (;l2 < end; l1 = l2, l2 = l2 + dl) {
            ...
            for (angle = 0; l1 < ol2; l1 += 2, l2 += 2) {
                ...
                dr = LLZ_FIXMUL_32X15(yr, wr) - LLZ_FIXMUL_32X15(yi, wi);
                di = LLZ_FIXMUL_32X15(yr, wi) + LLZ_FIXMUL_32X15(yi, wr);
...
uintptr_t llz_fft_fixed_init(int size)
{
    int i;
    double ang;
    llz_fft_fixed_ctx_t *f = NULL;

    f = (llz_fft_fixed_ctx_t *)malloc(sizeof(llz_fft_fixed_ctx_t));
    memset(f, 0, sizeof(llz_fft_fixed_ctx_t));
    ...
    for (i = 0 ; i < size ; i++) {
        ang = (2*M_PI*i)/size;
        f->cos_ang[i] = LLZ_FIX15(cos(ang));
        f->sin_ang[i] = LLZ_FIX15(sin(ang));
    }
